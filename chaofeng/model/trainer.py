...
from utils.functions import get_free_space_of_disk
from utils.functions import get_T5_config
from utils.functions import save_model_config
from utils.functions import my_average
...
class ChatTrainer:
    ...
    def train(self, is_keep_training: bool=False, is_finetune: bool=False, )->None:
        ...
        if accelerator.is_main_process:
            log.info('memory available: {:.2f} GM, dist available: {:.2f} GM, keep_in_memory: {}'.
        ...
        steps_per_epoch = int(np.ceil(len(train_dataloader) / total_batch_size))
        eval_steps = int(np.ceil(len(valid_dataloder) / total_batch_size))

        if accelerator.is_main_process:
            log.info('train dataset size: {}, steps_per_epoch: {}, valid dataset size: {}, eval_steps: {}, num_workers: {}'.format(
                len(train_dataloader), steps_per_epoch, len(valid_dataloder), eval_steps, num_workers
            ), save_to_file=True)
